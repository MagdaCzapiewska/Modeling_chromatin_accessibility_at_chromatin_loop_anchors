...
if (length(args) < 1) {
  stop("Error: Loop ID not provided!\nUsage: Rscript extract_loop_correlations.R <loop_id>")
}
...
if (length(res_list) > 0) {
  ...
  # Save original summary data as TSV
  ...
  fwrite(final_dt_print, file.path(pops_dir, paste0("summary_", target_loop, "_init_1e-6_correlations.tsv")), sep = "\t")
  ...
  plot_correlation_lines <- function(data_subset, title_suffix, filename) {
    ...
    full_plot_path <- file.path(pops_dir, filename)
    # Saved as PDF (dpi parameter is removed since PDF is natively vector-based)
