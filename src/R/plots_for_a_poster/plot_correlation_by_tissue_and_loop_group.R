...
output_file <- file.path(".", "correlation_by_lineage_vertical_grouped.pdf")
correlation_dir <- file.path(resultsdir, "MGLMfit_GDM_cor", "real_data", "pops", "init_1e-6")

# 2. Podział pętli na grupy na podstawie 9 kolumn binarizowanych
...
loops_data[, loop_group := ifelse(has_any_one == 1, "Active Loops (>=1 state)", "Inactive Loops (all 0)")]

loop_group_map <- loops_data[, .(loop_id, loop_group)]

# 3. Definicja okien czasowych
time_windows <- c("00-02", "02-04", "04-06", "06-08", "08-10", "10-12", "12-14", "14-16", "16-18", "18-20")

# 4. Wczytanie i połączenie plików korelacji
cor_list <- list()
...
# Przygotowanie zmiennych (czyszczenie nazw tkanek)
...
# 5. Definicja wspólnego korzenia (root)
root_pops <- c(
...
# 6. Mapowanie klastrów do 5 linii rozwojowych
lineages <- list(
  Ectodermal = c(
    root_pops,
...
# 7. Generowanie listy wykresów (pętle aktywne i nieaktywne dosłownie obok siebie dla każdej tkanki)
plot_list <- list()

for (lineage_name in names(lineages)) {
  lineage_data <- all_cor[tw_pop %in% lineages[[lineage_name]]]
  
  # KROK KLUCZOWY: Używamy interaction(), aby zgrupować tkanki w obrębie okien czasowych na osi X,
  # a kolor 'fill' odpowiada bezpośrednio za porównanie pętli (aktywne vs nieaktywne).
  p <- ggplot(lineage_data, aes(x = interaction(time_window, tissue, lex.order = TRUE), y = spearman_rho, fill = loop_group)) +
    geom_boxplot(outlier.size = 0.2, alpha = 0.85, position = position_dodge(width = 0.8)) +
    # Podział na okna czasowe w poziomie, 'scales = "free_x"' ukrywa puste tkanki z innych okien
    facet_wrap(~ time_window, nrow = 1, scales = "free_x") + 
    labs(
      title = paste(lineage_name, "Lineage"),
      x = "Tissue within Time Windows",
      y = "Spearman's Rho",
      fill = "Loop Status"
    ) +
    scale_fill_manual(values = c("Active Loops (>=1 state)" = "#E41A1C", "Inactive Loops (all 0)" = "#377EB8")) +
    ...
    theme(
      ...
      # Customizacja etykiet osi X, aby pokazywały tylko nazwę tkanki (bo okno czasowe jest już w nagłówku faceta)
    ...
    # Funkcja podmieniająca pełne nazwy "okno.tkanka" na samą czytelną nazwę "tkanka" na osi X
    scale_x_discrete(labels = function(x) gsub("^[^.]+\\.", "", x))
  
  plot_list[[lineage_name]] <- p
}

# 8. Połączenie 5 paneli pionowo (jeden pod drugim) z jedną legendą na dole
final_panel <- wrap_plots(plot_list, ncol = 1) + plot_layout(guides = "collect")

# Zapis do pliku PDF - szeroki format (szerokość 22 cale), aby zmieścić tkanki obok siebie w oknach
pdf(output_file, width = 22, height = 24)
print(final_panel)
dev.off()

message(paste("Sukces! Wykres bezpośredniego porównania tkanek został zapisany w:", output_file))
