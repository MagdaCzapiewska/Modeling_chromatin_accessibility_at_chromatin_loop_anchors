...
# --- MOTYW Z DUŻYMI CZCIONKAMI NA PLAKAT ---
base_theme <- theme_minimal(base_size = 14) + # Podniesiona bazowa wielkość
  theme(
    plot.title.position = "plot",
    # --- Pogrubione tytuły wykresów, dodany bezpieczny margines na dole ---
    ...
    # --- Wyraźniejsze i pogrubione liczby na osiach X ---
...
# --- ETYKIETA WIERSZA (Napis po lewej) ---
p_label <- ggplot() +
  # --- POPRAWKA: Zmniejszono size z 6 na 5 ---
...
p5 <- ggplot() +
  ...
  # --- POPRAWKA: Wymuszenie pełnego zakresu osi X poprzez podanie limits ---
  scale_x_continuous(breaks = c(0, MU, 10000), labels = c("0", "5000", "10,000"), limits = c(0, X_MAX)) +
  scale_y_continuous(limits = c(0, 1.05)) +
  base_theme

# --- UKŁAD WYKRESÓW Z NAGŁÓWKIEM ---
layout <- (
