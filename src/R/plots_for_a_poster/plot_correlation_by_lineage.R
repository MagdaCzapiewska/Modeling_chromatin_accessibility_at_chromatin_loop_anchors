...
loops_data <- fread(loops_file) # wszystkie loop_id są w kolumnie loop_id

output_file <- file.path(".", "correlation_by_lineage.pdf")
...
# Tworzenie czystej nazwy tkanki (bez numeru klastra na początku)
all_cor[, tissue := gsub("^[0-9]+_", "", population)]
all_cor[, time_window := factor(time_window, levels = time_windows)]

# Tworzenie unikalnego identyfikatora okno_klaster do precyzyjnego mapowania linii
...
# 6. Generowanie listy wykresów przy użyciu pętli
plot_list <- list()
...
for (lineage_name in names(lineages)) {
  ...
  # Tworzenie wykresu pudełkowego (Boxplot) dla spearman_rho
  p <- ggplot(lineage_data, aes(x = time_window, y = spearman_rho, fill = tissue)) +
    ...
    theme(
      ...
      legend.position = "right",                    # Legenda wymuszona po prawej dla każdego wykresu
...
# 7. Złożenie wykresów w pionowy stos (1 kolumna, 5 wierszy) przy pomocy patchwork
# Ponieważ nie używamy `collect`, każdy wykres zachowa swoją własną dedykowaną legendę po prawej stronie.
final_panel <- wrap_plots(plot_list, ncol = 1)

# 8. Zapis do pliku PDF o odpowiednich proporcjach (wysoki format, aby zapobiec ściśnięciu)
pdf(output_file, width = 11, height = 17)
print(final_panel)
dev.off()

message(paste("Sukces! Wykres został zapisany w:", output_file))
