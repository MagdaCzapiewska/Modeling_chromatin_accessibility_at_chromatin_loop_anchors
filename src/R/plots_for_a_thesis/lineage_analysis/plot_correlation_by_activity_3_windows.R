...
output_file <- file.path(output_dir, "correlation_by_lineage_with_activity_union_3_windows.pdf")

# Ścieżka do katalogu na osobne pliki i jego utworzenie
single_plots_dir <- file.path(output_dir, "correlation_by_lineage_with_activity_union_3_windows")
...
for (p_idx in seq_along(paths)) {
  ...
  p <- ggplot(plot_data, aes(x = x_label, y = spearman_rho, fill = split_var, color = split_var)) +
    ...
    # Poszerzenie osi Y o +0.35 dla klamer testu Wilcoxona
  ...
  if (has_data) {
    ...
    for (i in seq_along(path_full$x_label)) {
      ...
      # Test wykonujemy tylko wtedy, gdy w obu grupach są min. 3 obserwacje
      if (n0 >= 3 && n1 >= 3) {
        ...
        # Pozycja klamry w pionie (tuż nad najwyższym punktem w oknie)
        y_max_local <- max(sub_dt$spearman_rho, na.rm = TRUE)
        y_pos <- y_max_local + 0.08
        
        # Środki grup odsuwanych skrzypiec (dodge width = 0.8) mieszczą się w x = i - 0.2 oraz i + 0.2
        p <- p + geom_signif(
