...
# ==============================================================================
# 2. Definicja i opis profili A i B
# ==============================================================================
prof1_code <- "000010010" # Profile A
prof2_code <- "000000010" # Profile B
target_profiles <- c(prof1_code, prof2_code)
...
# ==============================================================================
# 3. Wczytanie i przygotowanie profili aktywności pętli
# ==============================================================================
message("Wczytywanie informacji o profilach pętli...")
act_dt <- fread(activity_file, select = c("loop_id", "small_profile"), colClasses = c(small_profile = "character"))

# Zapewnienie dokładnej długości 9 znaków z wiodącymi zerami
act_dt[, small_profile := str_pad(as.character(small_profile), width = 9, side = "left", pad = "0")]

# Filtrowanie tylko do pętli o wybranych profilach A i B
act_dt <- act_dt[small_profile %in% target_profiles]

# Przypisanie czytelnych nazw profilów
act_dt[, profile_group := ifelse(small_profile == prof1_code, prof1_label, prof2_label)]
act_dt[, profile_group := factor(profile_group, levels = c(prof1_label, prof2_label))]

# Wczytanie podstawowej listy pętli
loops_data <- fread(loops_file)

# ==============================================================================
# 4. Wczytywanie i filtrowanie danych korelacji
# ==============================================================================
message("Wczytywanie danych korelacji...")
cor_list <- list()
...
# Filtrowanie po dobrych dofitowaniach (min_est_over_se >= 5) i obecności w pliku pętli
all_cor <- all_cor[loop_id %in% loops_data$loop_id]
all_cor <- all_cor[min_est_over_se >= 5]

# Dołączenie informacji o profilu (tylko pętle posiadające Profile A lub B)
all_cor <- merge(all_cor, act_dt, by = "loop_id", all.x = FALSE)

# --- USUNIĘCIE NA: filtrowanie brakujących wartości spearman_rho i grupy profili ---
all_cor <- all_cor[!is.na(spearman_rho) & !is.na(profile_group)]

# Globalny zakres osi Y z zapasem na klamry testu Wilcoxona
global_y_min <- min(all_cor$spearman_rho, na.rm = TRUE)
global_y_max <- max(all_cor$spearman_rho, na.rm = TRUE)

# ==============================================================================
# 5. Generowanie wykresów dla ścieżek
# ==============================================================================
...
for (p_idx in seq_along(paths)) {
  ...
  # Połączenie korelacji na ścieżce
  cor_path <- merge(path_dt, all_cor, by = c("time_window", "population"), all.x = TRUE)
  
  # Usunięcie NA z korelacji na danej ścieżce
  cor_path <- cor_path[!is.na(spearman_rho) & !is.na(profile_group)]
  
  # Liczenie punktów dla obu grup profili w każdym oknie czasowym
  ...
  if (!prof1_label %in% names(counts_wide)) counts_wide[, (prof1_label) := 0]
  if (!prof2_label %in% names(counts_wide)) counts_wide[, (prof2_label) := 0]
  setnames(counts_wide, c(prof1_label, prof2_label), c("n_p1", "n_p2"))
  
  # Przygotowanie osi X z etykietami na 2 naprzemiennych poziomach populacji
  ...
  # Populacje na 2 poziomach (alternujące przesunięcia \n\n unikające nakładania się tekstu)
  ...
  p <- ggplot(cor_plot, aes(x = x_label, y = spearman_rho, fill = profile_group, color = profile_group)) +
    ...
    scale_fill_manual(values = setNames(c("#3498db", "#e74c3c"), c(prof1_label, prof2_label)), drop = FALSE) +
    scale_color_manual(values = setNames(c("#1f618d", "#922b21"), c(prof1_label, prof2_label)), drop = FALSE) +
    labs(
      ...
      subtitle = paste0("<b>Filtered by fit quality (min_est_over_se) &ge; 5</b><br>",
                        "<span style='font-size: 9.5pt; color: #444444;'>", prof1_desc, " &nbsp;|&nbsp; ", prof2_desc, "</span>"),
  ...
  if (has_data) {
    ...
    # --------------------------------------------------------------------------
    # Obliczanie testu Wilcoxona dla każdego okna czasowego (1..10)
    # --------------------------------------------------------------------------
    ...
    for (i in seq_along(labels_vec)) {
      ...
      n1 <- nrow(sub_dt[profile_group == prof1_label])
      n2 <- nrow(sub_dt[profile_group == prof2_label])
      
      # Test wykonywany gdy w obu grupach są co najmniej 3 obserwacje
  ...
  # Zapis do głównego pliku PDF
  print(p)
  
  # Zapis do osobnego pliku PDF
...
cat("2. Osobne pliki PDF w katalogu:", single_plots_dir, "\n")
