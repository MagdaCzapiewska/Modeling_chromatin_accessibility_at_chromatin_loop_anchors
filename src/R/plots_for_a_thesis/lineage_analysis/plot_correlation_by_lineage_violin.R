...
output_file <- file.path(output_dir, "correlation_by_lineage_violin.pdf")

# Ścieżka do katalogu na osobne pliki i jego utworzenie
single_plots_dir <- file.path(output_dir, "correlation_by_lineage_violin")
...
for (p_idx in seq_along(paths)) {
  ...
  p <- ggplot(cor_plot, aes(x = x_label, y = spearman_rho)) +
    ...
    labs(
      title = bquote(bold("Estimated Spearman's correlation along cell lineage (Path " * .(p_idx) * ")")),
      subtitle = "<b>Filter applied:</b> Included chromatin loops with fit quality metric (min EST/SE &ge; 5)",
      x = "Time window\nPopulation\nLineage\nNumber of loops (n)",
  ...
  if (has_data) {
    ...
    p <- p + 
      annotate("text", x = 5.5, y = mean(c(global_y_min, global_y_max)), 
               label = "No loops with min EST/SE ≥ 5", color = "red", fontface = "italic", size = 5)
  }
