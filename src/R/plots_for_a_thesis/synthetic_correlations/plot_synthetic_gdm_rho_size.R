...
setorder(stats_dt, rho, size_nb)

output_stats_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_matrix_rho_vs_size_N_", CURRENT_N, "_mu_", CURRENT_MU, "_stats.tsv.gz")
)
...
p_lines <- ggplot(stats_long, aes(x = size_nb, y = value, color = metric, group = metric)) +
  ...
  labs(
    title = bquote(bold("Evaluation of estimated Spearman's correlation (N = " * .(CURRENT_N) * ", " * mu * " = " * .(format_n(CURRENT_MU)) * ")")),
    subtitle = paste0(
      "<b>size:</b> Size parameter of the total count per observation distribution (<i>a priori</i>) | ",
...
output_lines_pdf_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_lines_rho_vs_size_N_", CURRENT_N, "_mu_", CURRENT_MU, ".pdf")
)
...
for (r in seq_along(RHOS)) {
  for (c in seq_along(SIZES)) {
    ...
    sub_dt <- dt[rho == current_rho & as.character(size_nb) == current_size]
    ...
    p <- ggplot(sub_dt, aes(x = spearman_rho, fill = color_group)) +
      ...
      matrix_theme(r, c, length(RHOS), length(SIZES))
...
combined_gdm <- wrap_plots(gdm_plots, ncol = length(SIZES), nrow = length(RHOS)) +
  plot_annotation(
    title = bquote(bold("Spearman's correlation between chromatin accessibility modeled by GDM distribution (N = " * .(CURRENT_N) * ", " * mu * " = " * .(format_n(CURRENT_MU)) * ")")),
    subtitle = paste0(
...
output_pdf_path <- file.path(OUTPUT_BASE_DIR, paste0("gdm_matrix_rho_vs_size_N_", CURRENT_N, "_mu_", CURRENT_MU, ".pdf"))
pdf(output_pdf_path, width = length(SIZES) * 2, height = length(RHOS) + 1.5)
print(combined_gdm)
dev.off()
