...
setorder(stats_dt, n, size_nb)

output_stats_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_matrix_N_vs_size_rho_", CURRENT_RHO, "_mu_", CURRENT_MU, "_stats.tsv.gz")
)
...
p_lines <- ggplot(stats_long, aes(x = size_nb, y = value, color = metric, group = metric)) +
  ...
  labs(
    title = bquote(bold("Evaluation of estimated Spearman's correlation (rho = " * .(CURRENT_RHO) * ", " * mu * " = " * .(format_n(CURRENT_MU)) * ")")),
    subtitle = paste0(
...
output_lines_pdf_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_lines_N_vs_size_rho_", CURRENT_RHO, "_mu_", CURRENT_MU, ".pdf")
)
...
for (r in seq_along(N_CELLS)) {
  for (c in seq_along(SIZES)) {
    ...
    sub_dt <- dt[n == current_n & as.character(size_nb) == current_size]
    ...
    p <- ggplot(sub_dt, aes(x = spearman_rho, fill = color_group)) +
      ...
      matrix_theme(r, c, length(N_CELLS), length(SIZES))
...
combined_gdm <- wrap_plots(gdm_plots, ncol = length(SIZES), nrow = length(N_CELLS)) +
  plot_annotation(
    title = bquote(bold("Spearman's correlation between chromatin accessibility modeled by GDM distribution (rho = " * .(CURRENT_RHO) * ", " * mu * " = " * .(format_n(CURRENT_MU)) * ")")),
    subtitle = paste0(
...
output_pdf_path <- file.path(OUTPUT_BASE_DIR, paste0("gdm_matrix_N_vs_size_rho_", CURRENT_RHO, "_mu_", CURRENT_MU, ".pdf"))
pdf(output_pdf_path, width = length(SIZES) * 2, height = length(N_CELLS) + 1.5)
print(combined_gdm)
dev.off()
