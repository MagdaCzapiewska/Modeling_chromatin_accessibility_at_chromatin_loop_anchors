...
setorder(stats_dt, rho, n)

output_stats_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_matrix_rho_vs_N_mu_", CURRENT_MU, "_size_", CURRENT_SIZE, "_stats.tsv.gz")
)
...
p_lines <- ggplot(stats_long, aes(x = n, y = value, color = metric, group = metric)) +
  ...
  scale_x_continuous(breaks = N_CELLS, labels = format_n(N_CELLS)) +
  scale_color_manual(
    values = c(
  ...
  labs(
    title = bquote(bold("Evaluation of estimated Spearman's correlation (" * mu * " = " * .(CURRENT_MU) * ", size = " * .(CURRENT_SIZE) * ")")),
    subtitle = paste0(
      "<b>N:</b> Number of synthetic cells per dataset (<i>a priori</i>) | ",
    ...
    x = "N - number of synthetic cells per dataset (a priori)",
    y = "Metric value"
  ) +
...
output_lines_pdf_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_lines_rho_vs_N_mu_", CURRENT_MU, "_size_", CURRENT_SIZE, ".pdf")
)
...
for (r in seq_along(RHOS)) {
  for (c in seq_along(N_CELLS)) {
    ...
    title_text <- if (r == 1) paste0("N = ", format_n(current_n)) else ""
...
combined_gdm <- wrap_plots(gdm_plots, ncol = length(N_CELLS), nrow = length(RHOS)) +
  plot_annotation(
    title = bquote(bold("Spearman's correlation between chromatin accessibility modeled by GDM distribution (" * mu * " = " * .(CURRENT_MU) * ", size = " * .(CURRENT_SIZE) * ")")),
    subtitle = paste0(
      "<b>Columns:</b> <i>N</i> &ndash; number of synthetic cells per dataset (<i>a priori</i>)<br>",
...
output_pdf_path <- file.path(
  OUTPUT_BASE_DIR, 
  paste0("gdm_matrix_rho_vs_N_mu_", CURRENT_MU, "_size_", CURRENT_SIZE, ".pdf")
)
