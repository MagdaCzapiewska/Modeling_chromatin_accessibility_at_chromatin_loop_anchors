...
if (length(args) < 1) {
  stop("Error: Loop ID not provided!\nUsage: Rscript extract_loop_correlation_all.R <loop_id>")
}
...
# 4. Color map & legend labels definitions
color_map <- c(
  "10" = "#99FF99", 
  "5"  = "#FFFF99", 
  "2"  = "#FFCC99", 
...
# 5. Iterate over time windows files
res_list <- list()
...
if (length(res_list) > 0) {
  ...
  full_plot_path <- file.path(output_folder, paste0("plot_", target_loop, "_spearman_lines_all_colored.pdf"))
  ggsave(full_plot_path, plot = p, width = 11, height = 5)
