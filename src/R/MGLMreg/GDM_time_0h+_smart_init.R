...
if (length(args) < 4) {
  stop("Usage: Rscript GDM_time_0h+_smart_init.R <loop_id> <out_file> <all> <n_take>")
}
...
log_file <- file.path(output_dir, "GDM_time_0h+_smart_init_reg_all.log")
...
X_temp <- model.matrix(~ time, data = df)
init_values <- matrix(1e-12, nrow = ncol(X_temp), ncol = 2 * (ncol(Y_temp) - 1))
...
time_fit <- system.time({
  fit_reg <- tryCatch({
    ...
    res <- withCallingHandlers({
      ...
      MGLMreg(cbind(x_out, x_A2, x_A1) ~ time, data = df, dist = "GDM", init = init_values)
