...
if (length(args) < 3) {
  stop("Użycie: Rscript plot_loop_triangles_counts_pop_2_scales.R <loop_id> <time_window> <fit|reg> [log|log2|nolog]")
}
...
cat(sprintf("Generowanie histogramów (2 skale): Loop = %s | TW = %s | Tryb = %s | Skala = %s\n", 
...
if (length(prepared_data) == 0) {
  stop("Brak poprawnych danych do wygenerowania histogramów.")
}
...
dev.off()
cat(sprintf("Zapisano plik PDF z histogramami populacji (2 skale): %s\n", pdf_file))
