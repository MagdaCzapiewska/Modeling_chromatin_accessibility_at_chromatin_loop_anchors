...
if (length(args) < 4) {
  ...
  stop("Usage: Rscript cor_real_pops.R <tw> <init> <output_file_tsv_gz> <fit_base_dir>")
}
...
pop_map_file <- file.path(resultsdir, "cluster_pop_mapping.tsv.gz")
pop_map_dt   <- fread(pop_map_file, colClasses = c(cluster_id = "character", time_window = "character"))

tw_map     <- pop_map_dt[time_window == current_tw]
pop_lookup <- setNames(tw_map$population, tw_map$cluster_id)
...
for (loop in loop_ids) {
  ...
  for (pop in valid_pops) {
    ...
    #  paste0(pop, "_unknown")
    #}
    full_pop_name <- pop_lookup[pop]
    if (is.na(full_pop_name)) full_pop_name <- pop
...
if (length(cor_results) > 0) {
  ...
  empty_template <- compute_gdm_correlations_from_fit(NULL, metadata_list = list(time_window=current_tw, population=NA_character_, loop_id=NA_character_))
