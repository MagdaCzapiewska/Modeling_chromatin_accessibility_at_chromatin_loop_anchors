...
if (length(args) < 3) {
  stop("Usage: Rscript naive_cor_synthetic.R <rho_value> <output_file_tsv_gz> <synthetic_data_dir>")
}

rho_val        <- as.numeric(args[1])
output_file    <- args[2]
synthetic_dir  <- args[3]
...
cat("Finished naive correlation calculation for rho:", rho_str, "\n")
