...
if (length(args) < 4) {
  ...
  stop("Usage: Rscript cor_real_all.R <tw> <init> <output_file_tsv_gz> <fit_base_dir>")
}
...
if (length(cor_results) > 0) {
  ...
  empty_template <- compute_gdm_correlations_from_fit(NULL, metadata_list = list(time_window=current_tw, loop_id=NA_character_))
