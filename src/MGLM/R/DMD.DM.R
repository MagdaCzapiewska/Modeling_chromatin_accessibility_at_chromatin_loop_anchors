...
DMD.DM.fit <- function(data, init, weight, epsilon = 1e-08, maxiters = 150, display = FALSE) {
  ...
  # xj is of length a, k is of length b
  # outer(xj, k) is of size a * b, where outer(xj, k, ">")[i,t] equals TRUE if xj[i] > k[t]
  # weight is of size a. weight * outer(x_j, k, ">") is of size a * b (we multiply i-th row by weight_i)
  # colSums cause that result is of length b. result[t] is the sum of weights of observations greater than k[t]
  # result[t] = sum_{i=1}^{a} weight[i] * 1(xj[i] > k[t])
  # When used with a = N and b = max, outer(xj, k, ">") is of size N * max
  
  Sj_general <- function(xj, k, weight) { # length(xj) = a, length(k) = b, length(weight) = a
    #cat("---- Sj_general DEBUG ----\n")
    #cat("Input xj:", xj, "\n")
    #cat("Input k:", k, "\n")
    ...
    x_sorted <- xj[ord]
    w_sorted <- weight[ord]
    #cat("x_sorted:", x_sorted, "\n")
    #cat("w_sorted:", w_sorted, "\n")
  
    w_cumsum <- cumsum(w_sorted) # w_sorted[1], w_sorted[1] + w_sorted[2], ..., sum_{i=1}^{a} w_sorted[i]
    total_w <- w_cumsum[length(w_cumsum)] # sum_{i=1}^{a} w_sorted[i]
    #cat("w_cumsum:", w_cumsum, "\n")
    #cat("total_w:", total_w, "\n")
  
    idx <- findInterval(k, x_sorted) # findInterval 
    #cat("findInterval idx:", idx, "\n")
  
    #result <- total_w - ifelse(idx > 0, w_cumsum[idx], 0)
    #cat("result before zero-fix:", result, "\n")
    #result[idx == length(x_sorted)] <- 0
    #cat("result after zero-fix:", result, "\n")
    result <- numeric(length(k))

    for (t in seq_along(k)) {
      if (idx[t] > 0) {
        result[t] <- total_w - w_cumsum[idx[t]]
      } else {
        result[t] <- total_w
      }
    }
    #cat("result after a loop:", result, "\n")
  ...
  #N <- 10
  #d <- 3
  #data <- matrix(sample(0:5, N * d, replace = TRUE), nrow = N, ncol = d)
  #m <- rowSums(data)
  #weight <- rep(1, N)
  #max_val <- max(m)
  #k <- 0:(max_val - 1)
  
  #cat("Start Sj (original)...\n")
  #t1 <- system.time({
  #  s1 <- apply(data, 2, Sj, k = k, weight = weight)
  #  r1 <- Sj(m, k, weight = weight)
  #})
  #cat("End Sj (original)\n")
  #cat("Sj (original) time:", t1["elapsed"], "s\n\n")

  # -------------------------------
  #cat("Start Sj_general...\n")
  #t2 <- system.time({
  #  s2 <- apply(data, 2, Sj_general, k = k, weight = weight)
  #  r2 <- Sj_general(m, k, weight = weight)
  #})
  #cat("End Sj_general\n")
  #cat("Sj_general time:", t2["elapsed"], "s\n\n")
  
  #if (all(dim(s1) == dim(s2))) {
  #  cat("s1 and s2 have the same dimensions:", paste(dim(s1), collapse = " x "), "\n")
  #} else {
  #  cat("s1 and s2 DIMENSIONS DIFFER!\n")
  #  cat("s1 dims:", paste(dim(s1), collapse = " x "), "\n")
  #  cat("s2 dims:", paste(dim(s2), collapse = " x "), "\n")
  #}

  #if (length(r1) == length(r2)) {
  #  cat("r1 and r2 have the same length:", length(r1), "\n")
  #} else {
  #  cat("r1 and r2 LENGTHS DIFFER!\n")
  #  cat("r1 length:", length(r1), "\n")
  #  cat("r2 length:", length(r2), "\n")
  #}
  
  #if (isTRUE(all.equal(s1, s2)) && isTRUE(all.equal(r1, r2))) {
  #  cat("Sj and Sj_general gives equal results!\n")
  #} else {
  #  cat("Results differ!\n")
  #}
  ...
  #cat("\ns1 (Sj results):\n")
  #print(s1)
  #cat("\ns2 (Sj_general results):\n")
  #print(s2)
  #cat("\nr1 (Sj results for m):\n")
  #print(r1)
  #cat("\nr2 (Sj_general results for m):\n")
  #print(r2)
  
  #cat("Start Sj true (original)\n")
  s <- apply(data, 2, Sj_general, k = k, weight = weight) # data is of size N * d, s is of size max * d
  # s[t, j] = sum_{i=1}^{N} weight[i] * 1(data[i, j] > k[t])
  r <- Sj_general(m, k, weight = weight) # m is of size N, r is of size max
  # r[t] = sum_{i=1}^{N} weight[i] * 1(m[i] > k[t])
  #cat("End Sj true (original)\n")
  alpha_hat <- init
  niter <- 1
