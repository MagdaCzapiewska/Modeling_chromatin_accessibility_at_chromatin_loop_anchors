...
RSCRIPT = os.path.join(config['paths']['Rsrcdir'], "MGLMfit_GDM_cor", "cor_real_all.R")
...
rule calculate_real_all_cor:
    ...
    output:
        os.path.join(OUT_REAL_ALL, "init_{init}", "cor_{tw}.tsv.gz")
    log:
        os.path.join(OUT_REAL_ALL, "logs", "cor_{tw}_init_{init}.log")
    shell:
        """
